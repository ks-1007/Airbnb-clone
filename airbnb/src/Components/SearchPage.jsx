...
function SearchPage() {
  ...
  const handleReview = () => {
    ...
    setValue(newValue);
    const st = filterHotel.filter((o) => o.price >= value[0] && o.price <= value[1]);
    setHotel(st);
  };

  const handleLike = async (likedData) => {
    await axios.patch(`http://localhost:3001/data/${likedData.id}`, {
      ...likedData,
  ...
  return (
    <div>
      <div className="searchPage__info">
        ...
        <hr className="searchPage__info_breakLine" />
      </div>
      {hotel.map((el) => (
        <div key={el.id}>
          <SearchResult
